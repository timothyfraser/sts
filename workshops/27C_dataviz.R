...
g = g %>% 
  add_layout(by = c("name", "type", "geography"), layout = "fr")
...
gg = gg +
  ...
  scale_fill_manual(
    breaks = c("Members", "Iwate Committees", "Miyagi Committees", "National Committees"),
...
# 4. Two Sided Bipartite Visual ##############################

## 4.1 Two-Sided Layout ############################
...
gg = gg + 
  geom_segment(data = highlight_edges,
               mapping = aes(x = from_x, y= from_y,
                             xend = to_x, yend = to_y),
...
gg = gg +
  # Plot an outline around the member of interest..
  geom_point(
    data = highlight %>% filter(type == FALSE),
    mapping = aes(x = x, y = y, size = deg),
    color = "black", shape = 21, fill = NA 
  ) +
  # Plot an outline around the COMMITTEE of interest..
  geom_point(
...
gg = gg + 
  geom_label(
    ...
    alpha = 0.5,# Make it transparent, so we see the graph below    
