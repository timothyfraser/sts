#' @name workflow3.R
#' @title Workflow for a spatial app with interactivity
#' @description
...
gg_map = ggplot() +
  ...
  # EDIT 2 ------------------------
  # Remove ggspatial - ggplotly only supports raw ggplot
...
# You may need to test out several versions in workflow before applying to app,
# because ggplotly doesn't always play nice with external packages (eg ggspatial, or sometimes on rare occasions, sf)
ggplotly(p = ggplot() + geom_sf(data = polygons_poi) )
ggplotly(p = ggplot() + geom_sf(data = poi) )

# This is the issue - plotly doesn't know what to do with a multilinestring, I guess.
# ggplotly(p = ggplot() + geom_sf(data = streets_box) )

# So let's simplify our plot...
