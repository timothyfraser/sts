#' @name app3.R
#' @author Tim Fraser
#' @title nycflights app V3
#' @description 
#' 
#' But, it's going to be much easier for us if we use `bslib`'s `card()` functions.
#' `bslib` is a package that brings the simplicity of 'bootstrap' web development to shiny.
#' Majorly simplifies the ui-making process.
#' Involves `bslib` functions including:
#' 
#' - `bslib::page()` --> replacement for `shiny::fluidPage()`
#' - `bslib::card()` 
#' - `bslib::card_header()`
#' - `bslib::card_footer()`
#' - `bslib::card_body()`
