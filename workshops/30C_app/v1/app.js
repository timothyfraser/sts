...
Shiny.addCustomMessageHandler('updateDegDensityChart', function(data) {
  ...
  // Build simple KDE-style density from sorted arrays
  function kernelDensity(vals, bandwidth) {
    const n = vals.length;
    ...
    return Array.from({ length: points }, (_, i) => {
      const x = min + i * step;
      const density = vals.reduce((sum, v) => {
        const u = (x - v) / bandwidth;
        return sum + Math.exp(-0.5 * u * u) / (Math.sqrt(2 * Math.PI) * bandwidth);
      }, 0) / n;
      return { x: parseFloat(x.toFixed(1)), y: parseFloat(density.toFixed(4)) };
    });
  }

  const bw = 1.5;
  const degPts  = kernelDensity(data.deg_values,  bw);
  const wdegPts = kernelDensity(data.wdeg_values, bw);
  const labels  = degPts.map(p => p.x);

  // Vertical line for median
  const medX = data.median_deg;
  ...
  mkChart('degDensityChart', {
    ...
    options: {
      ...
      scales: {
        ...
        y: { title: { display: true, text: 'Density', font: { size: 10 } }, ticks: { font: { size: 9 } }, grid: { color: 'rgba(0,0,0,0.04)' } }
      }
    }
  });
});
