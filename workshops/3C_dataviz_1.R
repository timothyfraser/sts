# class_3_dataviz_1.R
# Class 3: Intro to ggplot2 in R
# Topic: Data Visualization 1
# Prof: Tim Fraser

# In today's workshop, we will learn how to code visualizations, using the ggplot2 package.
...
# It is contained in data/gapminder.sqlite (also in the gapminder package).

# Let's connect, view, and collect the gapminder table (see console below)

# Option 1: Connect via sqlite
# db = dbConnect(drv = RSQLite::SQLite(), "data/gapminder.sqlite")
# Get gapminder data
# gapminder = db %>% tbl("gapminder") %>% collect()
# Disconnect
...
# but they are **groups of points** connected by a line.
...
mylines = ggplot() +
  ...
  geom_line(
    data = gapminder,
    # Add aesthetics, grouped by country
...
# This is the main logic of geom_line() compared to geom_point().
